...
int parse_command_line(int argc, char* argv[])
{
  ...
  * ( log_debug->ival ) = 0;
  ...
  char *log_file_pointer = (char *)malloc(strlen(TEST_TRAFFIC_LOG)+1);
    ...
    {
      arg_print_errors ( stdout, end, "Firewall" );
      printf ( "Firewall:\n Syntax and help:\n" );
      arg_print_glossary ( stdout, argtable, "%-43s %s\n" );
      exit (1);
    }
...
void* iptables_check_thread (void *ptr)
{
  ...
  printf ("Firewall has detected that some other process has changed\n");
...
void additional_iptable_config(){
_system("IP=\"\" ");
_system("SSH_IP=\"\"");
_system("#放行 ESTABLISHED,RELATED 状态");
_system("iptables -A INPUT -m state --state ESTABLISHED, RELATED -j ACCEPT ");
_system("iptables -A OUTPUT -m state --state ESTABLISHED, RELATED -j ACCEPT");
_system("");
_system("#INVALID 状态的日志");
_system("iptables -A INPUT -m state --state INVALID -j LOG ");
_system("iptables -A INPUT -m state --state INVALID -j DROP ");
_system("iptables -A OUTPUT -m state --state INVALID -j LOG ");
_system("iptables -A OUTPUT -m state --state INVALID -j DROP");
_system("#访问远程 SSH 服务器");
_system("iptables -A OUTPUT -p tcp -s $IP --sport 1024:65535 --dport 22 -m state —state NEW -j ACCEPT ");
_system("#对本地 SSH 服务器的远程访问，只准许部分 IP 进入");
_system("for i in $SSH_IP; ");
_system("do ");
_system("iptables -A INPUT -p tcp -s $SSH_IP --sport 1024:65535 -d $IP --dport 22 -m state --state NEW –j ACCEPT ");
_system("done");
}
