...
void init_conntrack(){
  ...
  //该函数注册一个回调来处理接收到的 conntrack，如果错误返回 -1 并适当设置 errno，否则返回 0。
  _nfct_callback_register (setmark_handle, NFCT_T_ALL, setmark, (void *)NULL);
  //Linux 中的 pthread_create() 函数用来创建线程
  //&tcp_export_thr 一种用于表示线程的数据类型
  //tcp_export_thread  以函数指针的方式指明新建线程需要执行的函数
